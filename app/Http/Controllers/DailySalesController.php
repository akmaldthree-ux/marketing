...
class DailySalesController extends Controller
{
    public function index(Request $request)
    {
        ...
        $dailySales = Order::whereIn('store_id', $storeIds)->where('status', 'complete')
            ->whereBetween('date', [$startDate, $endDate])
            ->select(DB::raw('date(date) as day'), DB::raw('sum(gmv) as total'), DB::raw('count(*) as orders'))
            ->groupBy('day')->orderBy('day')->get();

        $today = Order::whereIn('store_id', $storeIds)->where('status', 'complete')->whereDate('date', today())->sum('gmv');
        $yesterday = Order::whereIn('store_id', $storeIds)->where('status', 'complete')->whereDate('date', today()->subDay())->sum('gmv');
        $thisWeek = Order::whereIn('store_id', $storeIds)->where('status', 'complete')->whereBetween('date', [now()->startOfWeek(), now()->endOfWeek()])->sum('gmv');
        $lastWeek = Order::whereIn('store_id', $storeIds)->where('status', 'complete')->whereBetween('date', [now()->subWeek()->startOfWeek(), now()->subWeek()->endOfWeek()])->sum('gmv');

        $storeBreakdown = Store::whereIn('id', $storeIds)->get()->map(function ($store) use ($startDate, $endDate) {
            return [
                'store' => $store,
