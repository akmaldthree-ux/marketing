...
<div class="card">
  <div class="card-body p-0">
    <table class="table table-hover mb-0">
      <thead class="table-light"><tr><th>Toko</th><th>SKU</th><th>Nama Produk</th><th class="text-end">HPP/unit</th><th>Berlaku dari</th><th class="text-end">Aksi</th></tr></thead>
      <tbody>
      @forelse($cogs as $cog)
      <tr>
        <td class="small">{{ $cog->store?->name ?? '-' }}</td>
      ...
      <div class="modal fade" id="editModal{{ $cog->id }}">
        <div class="modal-dialog"><div class="modal-content">
          ...
          <div class="modal-header"><h6 class="modal-title">Edit HPP</h6><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
          <div class="modal-body">
            <div class="mb-2"><label class="form-label small">Toko</label>
              <select name="store_id" class="form-select form-select-sm" required>
                @foreach($stores as $s)<option value="{{ $s->id }}" {{ $cog->store_id==$s->id?'selected':'' }}>{{ $s->name }}</option>@endforeach
              </select>
            </div>
      ...
      @empty
      <tr><td colspan="6" class="text-center text-muted py-4"><i class="bi bi-tag fs-3 d-block mb-2 opacity-50"></i>Belum ada data HPP</td></tr>
      @endforelse
      </tbody>
    </table>
  </div>
...
<div class="modal fade" id="addModal">
  <div class="modal-dialog"><div class="modal-content">
    ...
    <div class="modal-body">
      ...
      <div class="mb-2"><label class="form-label small">SKU</label><input type="text" name="product_sku" class="form-control form-control-sm" required></div>
