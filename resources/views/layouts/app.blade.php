...
<head>
    ...
    <style>
        :root { --sidebar-bg:#0D1526;--sidebar-width:240px;--primary:#2563EB;--success:#16A34A;--danger:#DC2626;--warning:#D97706;--font-main:'DM Sans',sans-serif;--font-mono:'DM Mono',monospace; }
        * { font-family: var(--font-main); }
        body { background: #F1F5F9; min-height: 100vh; }
        .sidebar { width:var(--sidebar-width);background:var(--sidebar-bg);min-height:100vh;position:fixed;top:0;left:0;z-index:1000;display:flex;flex-direction:column;overflow-y:auto; }
        ...
        .sidebar-nav .nav-link { color:rgba(255,255,255,0.65);border-radius:8px;padding:0.55rem 0.85rem;font-size:0.85rem;font-weight:500;display:flex;align-items:center;gap:0.6rem;transition:all 0.15s; }
        ...
        .main-header { background:#fff;border-bottom:1px solid #E2E8F0;padding:0.75rem 1.5rem;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:500; }
        ...
        @media (max-width:768px) { .sidebar{transform:translateX(-100%);transition:transform 0.3s;} .sidebar.open{transform:translateX(0);} .main-wrapper{margin-left:0;} }
    </style>
    @yield('head')
</head>
<body>
    <nav class="sidebar" id="sidebar">
        ...
        <div class="sidebar-nav mt-2 flex-grow-1">
            ...
            <div class="nav-item"><a href="{{ route('dashboard') }}" class="nav-link {{ request()->routeIs('dashboard') ? 'active' : '' }}"><i class="bi bi-speedometer2"></i> Overview</a></div>
            <div class="nav-item"><a href="{{ route('daily-sales') }}" class="nav-link {{ request()->routeIs('daily-sales') ? 'active' : '' }}"><i class="bi bi-bar-chart-line"></i> Daily Sales</a></div>
            <div class="nav-item"><a href="{{ route('roas') }}" class="nav-link {{ request()->routeIs('roas') ? 'active' : '' }}"><i class="bi bi-bullseye"></i> ROAS Detail</a></div>
            <div class="sidebar-section mt-2">Performa</div>
            <div class="nav-item"><a href="{{ route('pic-performance') }}" class="nav-link {{ request()->routeIs('pic-performance') ? 'active' : '' }}"><i class="bi bi-people"></i> PIC Performance</a></div>
            <div class="nav-item"><a href="{{ route('funnel') }}" class="nav-link {{ request()->routeIs('funnel') ? 'active' : '' }}"><i class="bi bi-filter"></i> Funnel Analytics</a></div>
            <div class="nav-item"><a href="{{ route('pnl') }}" class="nav-link {{ request()->routeIs('pnl') ? 'active' : '' }}"><i class="bi bi-calculator"></i> P&amp;L Report</a></div>
            <div class="sidebar-section mt-2">Perencanaan</div>
            <div class="nav-item"><a href="{{ route('demand-forecast') }}" class="nav-link {{ request()->routeIs('demand-forecast') ? 'active' : '' }}"><i class="bi bi-boxes"></i> Demand Forecast</a></div>
            <div class="nav-item"><a href="{{ route('customer') }}" class="nav-link {{ request()->routeIs('customer') ? 'active' : '' }}"><i class="bi bi-person-heart"></i> Customer &amp; ROR</a></div>
            <div class="sidebar-section mt-2">Manajemen</div>
            <div class="nav-item"><a href="{{ route('upload') }}" class="nav-link {{ request()->routeIs('upload') ? 'active' : '' }}"><i class="bi bi-cloud-upload"></i> Upload Data</a></div>
            @if(auth()->user()->isAdmin())
            <div class="nav-item"><a href="{{ route('targets') }}" class="nav-link {{ request()->routeIs('targets') ? 'active' : '' }}"><i class="bi bi-trophy"></i> Target Management</a></div>
            <div class="nav-item"><a href="{{ route('settings') }}" class="nav-link {{ request()->routeIs('settings') ? 'active' : '' }}"><i class="bi bi-gear"></i> Settings</a></div>
            @endif
        </div>
        <div class="p-3 border-top" style="border-color:rgba(255,255,255,0.08)!important;">
            <div class="d-flex align-items-center gap-2">
                <div style="width:32px;height:32px;background:rgba(255,255,255,0.1);border-radius:50%;display:flex;align-items:center;justify-content:center;"><i class="bi bi-person text-white" style="font-size:0.9rem;"></i></div>
                <div>
                ...
                <form action="{{ route('logout') }}" method="POST" class="ms-auto">@csrf<button type="submit" class="btn btn-sm p-1" style="color:rgba(255,255,255,0.4);" title="Logout"><i class="bi bi-box-arrow-right"></i></button></form>
            </div>
        </div>
    </nav>
    <div class="main-wrapper">
        <header class="main-header">
            <div>
                ...
                <nav aria-label="breadcrumb"><ol class="breadcrumb mb-0"><li class="breadcrumb-item"><a href="{{ route('dashboard') }}" class="text-muted text-decoration-none">Home</a></li>@yield('breadcrumb')</ol></nav>
            </div>
        ...
        <div class="content-area">
            ...
            @if(session('error'))<div class="alert alert-danger alert-dismissible fade show mb-3"><i class="bi bi-exclamation-circle me-2"></i>{{ session('error') }}<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>@endif
            @yield('content')
        </div>
    </div>
    <div class="offcanvas offcanvas-end" id="notifPanel" style="width:360px;">
        <div class="offcanvas-header border-bottom"><h6 class="offcanvas-title fw-bold">Notifikasi</h6><button type="button" class="btn-close" data-bs-dismiss="offcanvas"></button></div>
        <div class="offcanvas-body p-0">
            ...
            <div class="p-3 border-bottom {{ !$n->is_read ? 'bg-light' : '' }}">
                <div class="d-flex gap-2">
                    <span class="mt-1">@if($n->type==='alert')<i class="bi bi-exclamation-circle-fill text-danger"></i>@elseif($n->type==='warning')<i class="bi bi-exclamation-triangle-fill text-warning"></i>@elseif($n->type==='success')<i class="bi bi-check-circle-fill text-success"></i>@else<i class="bi bi-info-circle-fill text-primary"></i>@endif</span>
                    <div><div class="fw-semibold" style="font-size:0.83rem;">{{ $n->title }}</div><div class="text-muted" style="font-size:0.78rem;">{{ $n->message }}</div><div class="text-muted mt-1" style="font-size:0.72rem;">{{ $n->created_at->diffForHumans() }}</div></div>
                </div>
            </div>
            @empty<div class="text-center text-muted p-4">Tidak ada notifikasi</div>
            @endforelse
        </div>
    </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.3/dist/chart.umd.min.js"></script>
    <script>Chart.defaults.font.family='DM Sans';Chart.defaults.color='#64748B';</script>
    @yield('scripts')
</body>
</html>
